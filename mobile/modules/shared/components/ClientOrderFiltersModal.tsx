...
export const DEFAULT_CLIENT_ORDER_FILTERS: ClientOrderFilters = {
  status: null,
  dateFrom: null,
  dateTo: null,
};

interface ClientOrderFiltersModalProps {
  visible: boolean;
  initialFilters: ClientOrderFilters;
  onClose: () => void;
  onApply: (filters: ClientOrderFilters) => void;
}
...
export const ClientOrderFiltersModal = ({
  visible,
  initialFilters,
  onClose,
  onApply,
}: ClientOrderFiltersModalProps) => {
  const theme = useTheme();
  const styles = useThemedStyles(createStyles);

  const [status, setStatus] = useState<OrderStatus | null>(initialFilters.status);
              ...
              {STATUS_OPTIONS.map((opt) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => setStatus(active ? null : opt.value)}
                    activeOpacity={0.8}
                  >
